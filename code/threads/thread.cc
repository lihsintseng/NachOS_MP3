...
Thread::Thread(char* threadName, int threadID)
{
    ...
    //new init
    //
    startburst=kernel->stats->totalTicks;	//start burst time
    bursttime=0;	//burst time
    prior=0;		//priority  default?
    totalexec=0;
    //
    ID = threadID;
    name = threadName;
...
Thread::Yield ()
{
	...
	if(nextThread->getPriority()>=50&&nextThread->getPriority()<100){
    		if((kernel->stats->totalTicks-nextThread->getBurstStart())>100){
        		temp_pri = nextThread->getPriority();
                	nextThread -> setPriority(10);
          	     	cout << "Tick " << kernel->stats->totalTicks << ": Thread " <<nextThread->getID() << " changes its priority from " << temp_pri  <<" to 10 "<< endl;
			//kernel->scheduler->ReadyToRun(nextThread);
		}
	}
	else if(nextThread->getPriority()>=100){
		if((kernel->stats->totalTicks-nextThread->getBurstStart())>100){
                        temp_pri = nextThread->getPriority();
                        nextThread -> setPriority(50); 
                        cout << "Tick " << kernel->stats->totalTicks << ": Thread " <<nextThread->getID() << " changes its priority from " << temp_pri  <<" to 50 "<< endl;
...
Thread::setPriority(int priority)
{
    ...
    else if(priority>149){
	prior = 149;
	return true;	
    }
    prior = priority; //set prior in function
//    cout<< "Tick " << kernel->stats->totalTicks << " Thread" << ID << " changes its priority to " << prior <<endl;
    return true;	
}
...
Thread::getPriority()
{
	return prior;	//return priority of thread
}
//
// set ready time
//
void
Thread::setReady(int readytime)
{
    startready=readytime;
}
...
void
Thread::setTotalExe(int exectime)
{
    totalexec=exectime;
}
...
//
bool
Thread::setBurst(double in_bursttime)
{
    bursttime = in_bursttime;
...
//
//set start burst time
//
bool
Thread::setBurstStart(int bs_time)
{
    if(bs_time<0) return false;
    startburst = bs_time;
    return true;
}
...
/*
int 
Thread::compare_by_priority(Thread* t1, Thread* t2)
{
        int prior1 = t1->getPriority();
	int prior2 = t2->getPriority();
...
/*
int 
Thread::compare_by_burst(Thread* t1, Thread* t2)
{
	int prior1 = t1->getBurst();
	int prior2 = t2->getBurst();
        if(prior1<prior2)return -1;
        if(prior1>prior2)return 1;
        return 0;	
}
