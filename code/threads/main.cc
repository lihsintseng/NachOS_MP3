...
/*
            ...
            ASSERT(i + 1 < argc);   // parameter name string
            parameterName = argv[i + 1];  
	    i++;
	    ASSERT(i + 1 < argc);   //parameter priority num
	    parameterPriority = atoi(argv[i+1]);
	    i++;
	    threadPriorityFlag = TRUE;	    
	}
*/
//
#ifndef FILESYS_STUB
...
// threadPriorityFlag
/* 
   if (threadPriorityFlag) {
	kernel->currentThread->setPriority(parameterPriority);
    }
*/
//char
//
