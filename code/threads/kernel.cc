...
int Kernel::Exec(char* name)
{
	...
	cout << "Thread " << threadNum << " " << name << " (Pri: " << priority[threadNum-1] << ")" <<endl;
	fflush(stdout);
	t[threadNum] = new Thread(name, threadNum, priority[threadNum-1]);
	t[threadNum]->space = new AddrSpace();
//
	if(threadNum==2||threadNum==3) {
		t[threadNum]->setBurst(0);
		t[threadNum]->setBurstStart(stats->totalTicks);
	}
	else{
        	t[threadNum]->setBurst((stats->totalTicks - t[threadNum-1]->getBurstStart() + t[threadNum-1]->getBurst()) / 2.0 );
    		t[threadNum]->setBurstStart(stats->totalTicks);
	}
//
